...
<body class="bg-[#F8FAFC] text-slate-800 flex flex-col min-h-screen">
    ...
    <main class="flex-grow max-w-7xl mx-auto w-full px-6 py-10 flex gap-8">
        ...
        <section class="flex-1 bg-white rounded-3xl shadow-sm border border-slate-200 p-8">
            ...
            <form action="${pageContext.request.contextPath}/admin/boardList" method="GET" class="flex items-center gap-3 mb-8 bg-slate-50 p-4 rounded-2xl border border-slate-100 shadow-sm">
                <select name="searchType" class="select select-bordered w-32 font-bold bg-white text-sm">
                    <option value="title" ${searchType == 'title' ? 'selected' : ''}>제목</option>
                    <option value="writer" ${searchType == 'writer' ? 'selected' : ''}>작성자(닉네임)</option>
                </select>
                ...
                <table class="table table-zebra w-full text-center">
                    <thead class="bg-slate-50 text-slate-500 font-bold">
                        <tr>
                            ...
                            <th>작성일</th>
                            <th>조회수</th>
                            <th class="text-red-500">신고수</th> <th>상태</th>
                            <th>관리</th>
                        </tr>
                    </thead>
                    <tbody class="font-medium text-slate-600">
                        <c:forEach var="item" items="${list}">
                            <tr class="hover:bg-slate-50 transition-colors">
                                ...
                                <td class="text-left font-bold text-slate-800 w-1/3 truncate max-w-xs">
                                ...
                                <td>
                                    <c:choose>
                                        <c:when test="${item.STATUS == 'DELETED'}">
                                            <span class="badge badge-error badge-xs text-[10px] text-white font-bold py-2">삭제됨</span>
                                        </c:when>
                                        <c:otherwise>
                                            <span class="badge badge-ghost badge-sm font-bold py-2 text-slate-400">정상</span>
                                        </c:otherwise>
                                    </c:choose>
                                </td>
                                ...
                                <td>
                                    <c:if test="${item.STATUS == 'NORMAL'}">
                                        <button type="button" 
                                                onclick="deletePost('${item.SEQBOARDPOST}', '${item.TITLE}')" 
                                                class="btn btn-error btn-xs text-white px-3 font-bold border-none">
                                            삭제
                                        </button>
                                    </c:if>
                        ...
                        <c:if test="${empty list}">
                            <tr><td colspan="8" class="py-24 text-slate-400 font-bold text-center">조건에 맞는 게시글이 없습니다.</td></tr>
                        </c:if>
                    </tbody>
                </table>
            </div>
            ...
            <div class="flex justify-center mt-10">
                <div class="join shadow-sm border border-slate-200">
                    <c:if test="${currentPage > 1}">
                        <a href="${pageContext.request.contextPath}/admin/boardList?page=${currentPage - 1}&searchType=${searchType}&searchKeyword=${searchKeyword}&showDeleted=${showDeleted}" class="join-item btn btn-sm bg-white text-slate-600 border-none">«</a>
                    </c:if>
