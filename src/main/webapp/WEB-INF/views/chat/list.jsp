            ...
            <aside class="border-b lg:border-b-0 lg:border-r border-slate-200 bg-indigo-200 flex flex-col min-h-0">
                <div class="p-5 border-b border-slate-200 bg-white shrink-0">
                    ...
                    <div class="flex flex-wrap gap-2">
                        <a href="${pageContext.request.contextPath}/chat/list"
                           class="px-4 py-2 rounded-full text-sm font-medium transition ${empty category ? 'bg-slate-900 text-white' : 'border border-slate-300 bg-white hover:bg-slate-100'}">전체</a>

                        <a href="${pageContext.request.contextPath}/chat/list?category=0"
                           class="px-4 py-2 rounded-full text-sm font-medium transition ${category == 0 ? 'bg-slate-900 text-white' : 'border border-slate-300 bg-white hover:bg-slate-100'}">동행</a>

                        <a href="${pageContext.request.contextPath}/chat/list?category=1"
                           class="px-4 py-2 rounded-full text-sm font-medium transition ${category == 1 ? 'bg-slate-900 text-white' : 'border border-slate-300 bg-white hover:bg-slate-100'}">개인</a>
                    </div>
                </div>
                ...
                <div class="chat-scroll flex-1 min-h-0 overflow-y-auto p-4 space-y-3">
                    <c:forEach items="${roomList}" var="room">
                        ...
                        <a href="${roomUrl}"
                           class="block rounded-2xl p-4 shadow-sm transition ${room.active == 1 ? 'border-2 border-sky-500 bg-sky-50' : 'border border-slate-200 bg-white hover:bg-slate-50'}">
                            ...
                            <div id="chatMessageList" class="space-y-5">
                                ...
                                <c:forEach items="${messageList}" var="msg">
                                    <c:choose>
                                        ...
                                        <c:when test="${fn:contains(msg.detail, 'chat-system-msg')}">
                        ...
                        <div class="border-t border-slate-200 p-4 bg-white relative shrink-0">
                            <form id="chatSendForm" class="flex items-end gap-3">
                                ...
                                <div class="flex-1 relative">
                                    ...
                                    <textarea id="message" name="message" rows="1" placeholder="메시지를 입력하세요."
                                              class="w-full resize-none rounded-2xl border border-slate-300 bg-slate-50 px-4 py-3 text-sm focus:outline-none focus:ring-2 focus:ring-sky-400"></textarea>
                                </div>
...
<script>
    ...
    function appendMessage(data, isMine) {
        ...
        if (data.message && data.message.includes('chat-system-msg')) {
            const html = `
                <div class="chat-system-row">
                    \${data.message}
        ...
        let contentHtml = isMine ? `
            <div class="flex items-end justify-end gap-2">
                <div class="flex flex-col items-end min-w-fit">
                    \${unreadBadge}
                    <p class="text-[10px] text-slate-400">\${data.messageTime}</p>
                </div>
        ...
        const html = `
            <div class="flex \${isMine ? 'justify-end' : 'items-start gap-3'}">
                \${!isMine ? `
                    <div class="w-10 h-10 rounded-full bg-slate-200 shrink-0 overflow-hidden">
                        <img src="\${contextPath}/resources/upload/profile/\${profileImg}" class="w-full h-full object-cover">
                    </div>
                ` : ''}
